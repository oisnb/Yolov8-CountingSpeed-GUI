...
def de_lpr(coord,im0):
    ...
    lprnet.to(device)
    lprnet.load_state_dict(torch.load(r"D:\deeplearning\YOLOv8-license-plate-recognize\lprr\Final_LPRNet_model.pth"))
    prebs = lprnet(ims.to(device))  # classifier prediction
    prebs = prebs.cpu().detach().numpy()

    # 记得更换模型路径  CPU版本
    # state_dict = torch.load(r"D:\pan\桌面\YOLOv8-PySide6-GUI-new\lprr\Final_LPRNet_model.pth", map_location=torch.device("cpu"))
